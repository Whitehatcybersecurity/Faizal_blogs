      ...
      public function destinationStore(Request $request){
        ...
        if($request->hdDestinationId == null){
            ...
            if ($request->hasFile('fileDestinationImage')) {
                ...
                //     'destination_image' => $this->fileUpload($file, '/home/faizgshb/public_html/upload/destination/' . $destination->id, $fileName)
                // ]);
            ...
            $oldImage = $request->hdDestinationImage;
                if ($request->hasFile('fileDestinationImage')) {
                    @unlink($oldImage);
                    $files = $request->file('fileDestinationImage');
                ...
                Destination::findorFail($request->hdDestinationId)->update([
                    ...
                    ######## Live Path ######################
                    // 'destination_image' => ($request->hasFile('fileDestinationImage')) ? 
                    // $this->fileUpload($request->file('fileDestinationImage'), 
                    // '/home/faizgshb/public_html/upload/destination/' . $request->hdDestinationId, $fileName) : $oldImage,

                    ##################################################

                    'destination_image' => ($request->hasFile('fileDestinationImage')) ? 
                    $this->fileUpload($request->file('fileDestinationImage'), 
                    'upload/destination/' . $request->hdDestinationId, $fileName) : $oldImage,
